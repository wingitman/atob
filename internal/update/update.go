...
const defaultRepoURL = "https://github.com/wingitman/atob.git"
...
// Check ensures a source checkout exists, fetches its remotes, and returns the
...
func Check(cfg config.Config, currentCommit string, historyLimit int) Info {
	info := Info{UpdatesEnabled: !cfg.Updates.DisableChecks}
	if cfg.Updates.DisableChecks {
		return info
	}
...
func ensureRepoPath(cfg config.Config) (string, error) {
	if cfg.Updates.RepoPath != "" && isGitRepo(cfg.Updates.RepoPath) {
		return cfg.Updates.RepoPath, nil
	}
	if cwd, err := os.Getwd(); err == nil && isAtobRepo(cwd) {
		_ = config.RecordUpdateMetadata("", cwd)
		return cwd, nil
	}
	repoPath := filepath.Join(config.ConfigDir(), "atob-src")
...
func isAtobRepo(path string) bool {
	if !isGitRepo(path) {
		return false
	}
	...
	return strings.Contains(remote, "atob")
}
...
func writeUnixScript(req InstallRequest) (string, error) {
	dir := filepath.Join(os.TempDir(), "atob-updates")
...
Write-Host ""
Write-Host "atob update complete: $installed" -ForegroundColor Green
